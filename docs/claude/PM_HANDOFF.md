...
**A. Repository state (non-destructive, fail-fast)**

```bash
set -euo pipefail
git fetch origin
git switch main
[ -z "$(git status --short)" ] || { echo "DIRTY_WORKTREE"; exit 1; }
git pull --ff-only origin main
[ "$(git rev-parse HEAD)" = "$(git rev-parse origin/main)" ] || { echo "HEAD_NOT_ORIGIN_MAIN"; exit 1; }
git rev-parse HEAD
git status --short
git log --oneline -30
```

`git reset --hard`는 감사 절차에 포함하지 않는다. `set -euo pipefail`과 두
검증(clean tree, `HEAD == origin/main`)이 있으므로 `fetch`/`pull`/검증
실패 시 이후 명령이 stale/로컬-수정 상태에 대해 실행되지 않고 즉시
중단된다.
...
**C. Full deterministic suites (자체 test suite)**
...
`tests.ci.test_security_gates`/`tests.claude.test_policy_guard`는
**merge-gate 자체의 unit self-test**다 — 합성 fixture로 gate 로직 자체를
검증할 뿐, 지금 diff/HEAD에 대해 실제 gate를 실행한 결과가 아니다. 실제
latest-head 판정은 **C'**를 따로 실행해야 한다.
...
live 비활성/live authority 없음 확인 · tracked secret 없음 확인(**C'**
`scripts/ci/security_gates.py`의 `FORBIDDEN_TRACKED_PATH` gate 또는 CI의
`security-gates` workflow run 결과 기준 — C의 self-test 통과만으로는
tracked secret이 없다고 결론 내리지 않는다) · Python direct order path
없음 확인 · BingX public client에 private/account/order endpoint 없음
확인 · risk-policy 완화 없음 확인 · latest-head 기준 **C'**(`security-gates`
실제 실행 또는 CI workflow run) green 확인 · CodeRabbit review 완전성
확인(`docs/claude/CODERABBIT_REVIEW_MODEL.md` §9) · human merge.
CodeRabbit이 GitHub branch protection의 formal required
