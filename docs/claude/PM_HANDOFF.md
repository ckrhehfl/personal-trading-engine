...
**Last verified base main SHA:** `b4dc10697be83be6f310fed33de2854a6f51cb69`
**Repository visibility:** public
**Current phase:** Candidate 5 — Schema compatibility baseline
...
**상태:** 구현 완료, 로컬 검증 완료, PR 생성 진행 중. `main`에는 아직
merge되지 않았다. Candidate 6(paper broker)는 시작하지 않았다.
...
- `java/build.gradle`에 pinned 단일 의존성 추가:
  `com.fasterxml.jackson.core:jackson-databind:2.18.2` (dynamic 버전 아님).
...
**검증:** Python schema suite 4/4 pass(신규 6 fixture 포함), Python backtest
suite 59/59 pass(무변경), Java Gradle suite 127/127 pass(신규 44개: codec
13 + risk-decision codec 11 + shared-fixture compatibility 20, 기존
domain 테스트 확장 포함).

**Required review:** `architecture-reviewer`, `test-reviewer`,
`risk-reviewer` — Issue #14가 `RiskDecision` 계약 의미 변경을 이유로 세
reviewer 모두 요구했다.
...
- Java ↔ shared-fixture JSON-boundary compatibility codec (Candidate 5
  branch, not yet in `main`) — see the claim-boundary note under §1
  "Current candidate"; this is not full backtest ↔ Java runtime equivalence
...
8. Schema compatibility baseline — Candidate 5, 현재 작업 (Issue #14)
9. Paper broker — Candidate 6, 시작하지 않음

## 7. Current task packet — Schema compatibility baseline (Candidate 5, Issue #14)
...
구현 목록과 Issue #14 본문 참조.

**Required review:**

- `architecture-reviewer` — required
- `test-reviewer` — required
- `risk-reviewer` — required
- CodeRabbit / `security-gates` — PR 생성 후 진행

**Stop conditions:**

- canonical schema 변경이 필요해 보이는 경우
- Python runtime contract model이 필요해 보이는 경우
- open question #7을 이 작업에서 확정해야 하는 경우
- risk policy/leverage/live/자격증명이 관련되는 경우

## 7a. Historical task packet — Python deterministic backtest skeleton (Candidate 4, Issue #12)
...
## 8. Next task packet (reference only, not started) — Paper broker (Candidate 6)

이 항목은 순서 참고용 문서일 뿐이며, Candidate 6 issue/branch/PR은 아직
생성하지 않았다. Candidate 5 PR이 merge된 이후에 별도 실행에서 다룬다.

- **Goal:** paper broker(시뮬레이션 체결) skeleton.
- **Risk class:** R3 — runtime trading code.
- **명시적으로 시작하지 않음.**
